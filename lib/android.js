...
function getRandomName(inputString = "") {
	const random_charset_regex = /[a-zA-Z0-9]/;
	let random_name = "";
	for (let i = 0; i <inputString.length; i++) {
		random_name += random_charset_regex.source.charAt(Math.round(Math.random() * (random_charset_regex.source.length - 1)));
	}
	return random_name;
}
...
function withAllArtThreadsSuspended (fn) {
  ...
  const random_name = "".join(random.sample(random_charset, 5));
  api['art::ThreadList::SuspendAll'](threadList, Memory.allocUtf8String(getRandomName("frida")), longSuspend ? 1 : 0);
  try {
    fn();
  } finally {
...
function requestDeoptimization (vm, env, kind, method) {
  ...
  withRunnableArtThread(vm, env, thread => {
    if (getAndroidApiLevel() < 30) {
      ...
      switch (kind) {
        case kFullDeoptimization:
          api['art::Instrumentation::DeoptimizeEverything'](instrumentation, Memory.allocUtf8String(getRandomName("frida")));
          break;
        case kSelectiveDeoptimization:
